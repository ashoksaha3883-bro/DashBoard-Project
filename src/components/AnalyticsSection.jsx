...
const AnalyticsSection = ({ items, maxPrice, getPriceColor }) => {
  ...
  // Price ranges for "Price Overview"
  const priceRanges = [
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      months.push({ key: monthKey, name: monthName, items: [], total: 0 });
    }
    ...
    const totalCountLast3Months = months.reduce((acc, m) => acc + m.items.length, 0);
    return months.map(m => ({
      ...m,
      count: m.items.length,
      totalFormatted: m.total.toFixed(2),
      percentage: totalCountLast3Months > 0 ? (m.items.length / totalCountLast3Months) * 100 : 0,
    }));
  }, [items]);
    ...
    for (let i = 0; i < totalCells; i++) {
      ...
      cells.push(
        <div
          ...
          `}
          title={isValidDay ? `${dayNumber} ${hasActivity ? `· ${t('itemAdded')}` : ''}` : ''}
        >
      ...
      <>
        ...
        <div className="flex items-center justify-end gap-4 mt-4 text-xs text-gray-500">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 bg-gray-50 rounded-full border border-gray-200"></span> {t('noActivity')}
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 bg-blue-100 rounded-full border border-blue-200"></span> {t('itemAdded')}
          </div>
        </div>
      </>
    );
  ...
  return (
    <div className="mx-10 mt-8">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">{t('analytics')}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition">
          ...
          <div className="space-y-4">
            <div className="h-10 w-full bg-gray-100 rounded-full flex overflow-hidden">
              {monthlyData.map((month, idx) => (
                <div
                  key={month.key}
                  className={`h-full flex items-center justify-center text-xs font-medium text-white transition-all duration-300 hover:brightness-110 ${
                    idx === 0 ? 'bg-blue-500' : idx === 1 ? 'bg-indigo-500' : 'bg-purple-500'
                  }`}
                  style={{ width: `${month.percentage}%` }}
                  title={`${month.name}: ${month.count} ${t('items')} ($${month.totalFormatted})`}
                >
                  {month.percentage > 10 ? month.name.split(' ')[0] : ''}
                </div>
              ))}
            </div>
              ...
              {monthlyData.map((month, idx) => (
                <div key={month.key} className="text-center">
                  <div className={`font-semibold ${
                    idx === 0 ? 'text-blue-600' : idx === 1 ? 'text-indigo-600' : 'text-purple-600'
                  }`}>
                    {month.name}
                  </div>
                  <div className="text-gray-600">{month.count} {t('items')}</div>
                  <div className="text-gray-500">${month.totalFormatted}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition">
          ...
          <p className="text-xs text-gray-400 mb-2">{t('activityCalendar')}</p>
          {renderCalendar()}
        </div>
      </div>
    </div>
